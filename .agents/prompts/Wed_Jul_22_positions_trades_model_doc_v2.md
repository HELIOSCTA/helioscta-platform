...
The current operational issue is artifact drift: when product matching fails and the fix is made in the dbt/shared rule layer, every downstream SQL script that embeds compiled dbt logic must be refreshed. Frontend NAV and Clear Street SQL artifacts are promoted by `dbt/azure_postgres/scripts/promote_positions_trades_sql.py`. Backend MUFG and validation SQL artifacts are also promoted by that script. The NAV Excel/report layer has v2 dbt models under `dbt/azure_postgres/models/positions_and_trades_v2/nav_positions/excel/`, plus legacy extracted workbook SQL under `dbt/azure_postgres/reference_sql/ddl/nav/positions/excel_file/sql/`, but repo inspection shows no equivalent checked-in Excel SQL promotion step in `promote_positions_trades_sql.py`. Document that distinction clearly.
</context>
...
5. A generated-artifact matrix with columns for consumer, source dbt model, promoted/generated SQL path, promotion mechanism, and current drift risk.
...
- Treat `frontend/sql/...` and `backend/scrapes/positions_and_trades/sql/generated/...` as generated artifacts. Why: direct edits can diverge from dbt source-of-truth and be overwritten by `dbt compile` plus `promote_positions_trades_sql.py`.
- Do not claim Excel SQL is covered by `promote_positions_trades_sql.py` unless inspection proves it. Why: the current script's artifact list covers frontend/backend SQL, while Excel has v2 dbt models and legacy extracted SQL but no visible promotion target.
...
- The doc contains a checklist that future product-matching fixes can follow so frontend SQL, backend generated SQL, and Excel SQL do not drift.
