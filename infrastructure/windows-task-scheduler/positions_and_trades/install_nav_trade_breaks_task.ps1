# Installs or updates the local NAV trade-breaks Task Scheduler job.
#
# Task Scheduler starts one process daily. The Python orchestration polls for
# the target NAV Trade Breaks workbook, queues the templated email with the
...
function Assert-Config {
    ...
    if ($missing.Count -gt 0) {
        throw (
            "Production checkout is missing NAV trade-breaks schedule config: " +
            ($missing -join ", ") +
...
Write-Host "Installing NAV Trade Breaks Task Scheduler job"
...
if ($RunImportSmoke) {
    Invoke-External -FilePath $resolvedPythonExe -Arguments @(
        "-c",
        "from backend.orchestration.nav import trade_breaks_email; print('nav trade-breaks runtime import ok')"
    ) -WorkingDirectory $resolvedRepoRoot
}
...
$task = New-ScheduledTask `
    ...
    -Settings $settings `
    -Principal $principal `
    -Description "Polls for daily NAV trade-break workbooks and queues email notifications."
