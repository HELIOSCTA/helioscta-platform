"""EIA-930 daily generation by fuel type.

Source contract:
- Source system: EIA Open Data API v2, Hourly Electric Grid Monitor.
- Endpoint: /electricity/rto/daily-fuel-type-data
- Frequency: daily
- Raw grain: period x respondent x fueltype x timezone
- Destination: eia.eia_930_daily_generation_by_fuel
- Safe rerun: idempotent upsert on the source grain.

The public daily endpoint returns multiple timezone variants for each
period/respondent/fueltype. Preserve timezone in the raw table; downstream
curated marts can select a canonical timezone to match one-row-per-day website
contracts such as Edi's POWER_BY_FUEL_DAILY.
"""
...
def _pull(
    ...
    *,
    ...
    """Pull and normalize daily EIA-930 fuel-type rows."""
...
def _format(df: pd.DataFrame) -> pd.DataFrame:
    """Sanitize EIA daily fuel API response rows into the raw target shape."""
    if df.empty:
        return pd.DataFrame(columns=TARGET_COLUMNS)

    missing = [column for column in REQUIRED_SOURCE_COLUMNS if column not in df.columns]
    if missing:
        raise RuntimeError(f"EIA-930 daily payload missing required columns: {missing}")
    ...
    normalized["fueltype"] = _string_column(df, "fueltype")
...
def _upsert(
    ...
    """Upsert normalized EIA-930 daily rows into Azure Postgres."""
    if df.empty:
...
def main(
    ...
    """Run the EIA-930 daily generation-by-fuel scrape."""
