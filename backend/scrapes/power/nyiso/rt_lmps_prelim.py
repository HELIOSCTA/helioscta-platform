"""NYISO preliminary real-time five-minute zonal LBMPs."""
...
API_SCRAPE_NAME = "nyiso_rt_lmps_prelim"
ENDPOINT_INDEX_URL = _lmp.RT_INDEX_URL
ENDPOINT_TEMPLATE = f"{_lmp.RT_ENDPOINT}/{{operating_date:%Y%m%d}}realtime_zone.csv"
MARKET_RUN_ID = "RTD"
...
def endpoint_url_for_date(operating_date) -> str:
    return _lmp.rt_csv_url(operating_date)


def _resolve_default_start_date():
    return (
        pd.Timestamp.now(tz=LOCAL_MARKET_TIMEZONE)
        - relativedelta(days=DEFAULT_LOOKBACK_DAYS)
    ).date()
...
def _pull(
    *,
    ...
    """Pull NYISO preliminary real-time five-minute LBMPs for one operating date."""
...
def main(
    ...
    """Run the NYISO preliminary real-time five-minute zonal LBMP scrape."""
