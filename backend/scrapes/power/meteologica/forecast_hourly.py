"""Shared Meteologica hourly forecast scrape helpers."""
...
def pull_forecasts(
    *,
    ...
    scrape_run_at_utc = scrape_run_at_utc or utc_now()
...
def run_forecast_scrape(
    *,
    ...
    """Pull, upsert, and purge one ISO's configured forecast feeds."""
    ...
    run_id = str(uuid4())
    scrape_run_at_utc = utc_now()
    pull_fn = pull_fn or pull_forecasts
    if upsert_fn is None:
        upsert_fn = lambda df, database=None: upsert_forecasts(
            df,
    ...
    try:
        ...
        upsert_fn(df, database=database)
        deleted_rows = purge_fn(
            retention_days=retention_days,
            database=database,
        )
        run_logger.section(
        ...
        run_logger.success(f"{pipeline_name} completed; {len(df)} rows processed.")
        return df
