...
- Feed short name: rt_hrl_lmps_final
- Display name: Final Real-Time Hourly LMPs
- Category: Pricing Reports
- Description: Finalized locational marginal prices by hour for the Hub, load
  zones, and network nodes, with energy, congestion, and loss components.
- Posting frequency: Daily CSV reports after finalization.
"""
...
def _resolve_default_start_date() -> datetime:
    return datetime.now() - relativedelta(days=5)
...
def _build_url(start_date: datetime) -> str:
    endpoint = (
        "static-transform/csv/histRpts/rt-lmp/"
        f"lmp_rt_final_{start_date.strftime('%Y%m%d')}.csv"
    )
...
def _pull(
    *,
    ...
    """Pull ISO-NE final real-time hourly LMPs for one operating date."""
...
def main(
    ...
    """Run the ISO-NE final real-time hourly LMP scrape."""
