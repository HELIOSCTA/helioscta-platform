...
- Feed short name: da_hrl_cleared_demand
- Display name: Day-Ahead Hourly Cleared Demand Report
- Category: Load and Demand
- Description: Hourly ISO-NE day-ahead cleared demand in MW.
...
def _build_url(start_date: datetime, end_date: datetime) -> str:
    endpoint = (
        "transform/csv/hourlydayaheaddemand?"
...
def _pull(
    *,
    ...
    """Pull ISO-NE day-ahead cleared demand for one operating date."""
...
def _format(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["day_ahead_cleared_demand"] = pd.to_numeric(
        df["day_ahead_cleared_demand"],
        errors="coerce",
    )
...
def main(
    ...
    """Run the ISO-NE day-ahead cleared demand scrape."""
