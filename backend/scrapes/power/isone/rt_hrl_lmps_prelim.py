...
- Feed short name: rt_hrl_lmps_prelim
- Display name: Preliminary Real-Time Hourly LMPs
- Category: Pricing Reports
- Description: Preliminary locational marginal prices by hour with total LMP,
...
def _build_url(start_date: datetime) -> str:
    endpoint = (
        ...
        f"lmp_rt_prelim_{start_date.strftime('%Y%m%d')}.csv"
    )
...
def _pull(
    *,
    ...
    """Pull ISO-NE preliminary real-time hourly LMPs for one operating date."""
...
def main(
    ...
    """Run the ISO-NE preliminary real-time hourly LMP scrape."""
