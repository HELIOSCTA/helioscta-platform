...
The ISO-NE CSV endpoint is named `actualinterchange` and returns actual
interchange, purchases, and sales by hourly interface. The promoted table keeps
the legacy feed name `rt_hrl_scheduled_interchange` for continuity.
"""
...
PRIMARY_KEY = ["local_date", "local_hour_ending", "interface_name"]
...
def _build_url(start_date: datetime, end_date: datetime) -> str:
    endpoint = (
        "transform/csv/actualinterchange?"
...
def _pull(
    *,
    ...
    """Pull ISO-NE hourly actual scheduled interchange for one date."""
    end_date = end_date or start_date
    url = _build_url(start_date=start_date, end_date=end_date)
...
def _format(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["local_hour_ending"] = (
        pd.to_numeric(df["local_hour_ending"], errors="raise").astype(int)
    )
    df["interface_name"] = df["interface_name"].astype(str).str.strip()
    for col in ["actual_interchange", "purchases", "sales"]:
...
def main(
    ...
    """Run the ISO-NE hourly interchange scrape."""
