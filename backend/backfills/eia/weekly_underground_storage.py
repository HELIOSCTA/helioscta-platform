"""Manual backfill runner for EIA weekly underground storage."""
...
from backend.orchestration.eia import weekly_underground_storage as workflow


API_SCRAPE_NAME = workflow.API_SCRAPE_NAME
DEFAULT_START_DATE = datetime.now(timezone.utc).date() - timedelta(days=365)
DEFAULT_END_DATE = datetime.now(timezone.utc).date()
DEFAULT_MAX_DAYS = 3650
DEFAULT_CHUNK_DAYS = 730
...
def main(
    ...
    """Replay EIA weekly underground storage with idempotent upserts."""
