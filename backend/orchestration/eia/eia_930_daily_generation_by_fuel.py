"""Orchestrate EIA-930 daily generation-by-fuel refreshes."""
...
DEFAULT_METADATA = {
    "schedule_reason": "rolling_eia_930_daily_generation_refresh",
}
...
def main(
    ...
    """Run the promoted EIA-930 daily generation workflow."""
    ...
    try:
        ...
        if should_poll:
            ...
            run_logger.section(
                f"Waiting for daily EIA-930 rows for {resolved_target_date}..."
            )
...
def _poll_for_target_date(
    *,
    ...
    return _polling.poll_until_available(
        ...
        check_available=lambda df: _daily_availability(df, target_date),
        poll_ceiling_seconds=poll_ceiling_seconds,
        poll_wait_seconds=poll_wait_seconds,
        sleep_fn=sleep_fn,
    )
...
def _daily_availability(
    ...
    if df.empty:
        return _polling.AvailabilityResult(
            is_available=False,
            message=f"EIA-930 daily data is not available for {target_date}.",
    ...
    metadata = {
        ...
        "target_fueltype_count": int(target_df["fueltype"].nunique()),
        "target_timezone_count": int(target_df["timezone"].nunique()),
    }
    if target_row_count > 0:
        return _polling.AvailabilityResult(
            is_available=True,
            message="target EIA-930 daily data is available",
            metadata=metadata,
        )
    return _polling.AvailabilityResult(
        is_available=False,
