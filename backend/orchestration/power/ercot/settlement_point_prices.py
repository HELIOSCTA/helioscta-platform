"""Orchestrate ERCOT Real-Time Settlement Point Prices."""
...
from backend.scrapes.power.ercot import settlement_point_prices as scrape
...
DATASET_NAME = "ercot_settlement_point_prices"
DATA_SOURCE_SYSTEM = "ercot"
DATA_AVAILABILITY_TYPE = "data_ready"
DATA_SCOPE = "hub"
DATA_GRAIN = "date_15min_settlementpoint"
...
def main(
    ...
    """Run the ERCOT RT SPP workflow and emit complete-day readiness events."""
    now = datetime.now()
    ...
    try:
        ...
        if events:
            ...
            run_logger.info(
                "No complete ERCOT RT SPP business date detected; "
...
def _emit_data_availability_events(
    ...
    """Emit one readiness event per complete ERCOT RT delivery date."""
    if df.empty:
        logger.info("No ERCOT RT SPP rows available for readiness emission")
        return []
    ...
    if missing_columns:
        raise ValueError(
            "Cannot assess ERCOT RT SPP data readiness; missing columns: "
            f"{sorted(missing_columns)}"
        )

    current_df = df.copy()
    current_df["deliverydate"] = pd.to_datetime(current_df["deliverydate"]).dt.date
    current_df["deliveryhour"] = pd.to_numeric(
        current_df["deliveryhour"],
        errors="coerce",
    )
    current_df["deliveryinterval"] = pd.to_numeric(
        current_df["deliveryinterval"],
        errors="coerce",
    )
...
def _emit_data_availability_event_for_date(
    *,
    ...
    period_count = int(
        date_df[["deliveryhour", "deliveryinterval"]].drop_duplicates().shape[0]
    )
    periods_per_entity = date_df.groupby("settlementpoint")[
        ["deliveryhour", "deliveryinterval"]
    ].apply(
        lambda frame: frame[["deliveryhour", "deliveryinterval"]]
        .drop_duplicates()
        .shape[0]
    )
    ...
    duplicate_entity_period_rows = int(
        date_df.duplicated(
            ["settlementpoint", "deliveryhour", "deliveryinterval"]
        ).sum()
    )
    ...
    if not is_complete:
        logger.warning(
            "Skipping ERCOT RT SPP readiness event for %s; incomplete rows "
...
def _expected_period_count_for_date(business_date: date) -> int:
    ...
    return int((end - start) / pd.Timedelta(hours=1)) * PERIODS_PER_HOUR
