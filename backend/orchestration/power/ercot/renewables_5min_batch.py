"""Run promoted ERCOT 5-minute renewable actual scrape modules."""
...
def _run_feed(
    ...
    *,
    ...
    try:
        ...
        print(f"ERCOT 5-minute renewables feed failed: {feed_name}", flush=True)
        traceback.print_exc()
...
def main(
    ...
    """Run ERCOT 5-minute renewable actual feeds and return nonzero on failure."""
    _configure_logging()
    now = datetime.now()
    ...
    print(
        "Starting ERCOT 5-minute renewables batch at "
    ...
    for result in results:
        if result.status == "succeeded":
            print(
                "ERCOT 5-minute renewables feed succeeded: "
                f"{result.feed_name} in {result.elapsed_seconds:.1f}s",
                flush=True,
            )
        else:
            print(
                "ERCOT 5-minute renewables feed failed: "
                f"{result.feed_name} in {result.elapsed_seconds:.1f}s: {result.error}",
                flush=True,
            )

    print(
        "Completed ERCOT 5-minute renewables batch: "
