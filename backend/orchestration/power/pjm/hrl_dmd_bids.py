"""Orchestrate PJM hourly demand bid publication polling."""
...
from backend.scrapes.power.pjm import hrl_dmd_bids as scrape
...
EXPECTED_AREAS = frozenset({"PJM_RTO", "MID_ATLANTIC_REGION", "WESTERN_REGION"})
...
def _wait_for_complete_data(target_date: date) -> pd.DataFrame:
    ...
    if not shape["is_complete"]:
        raise DataNotYetAvailable(
            "PJM hrl_dmd_bids is not complete for "
            f"{target_date.isoformat()} "
            f"(rows={shape['row_count']}, areas={shape['areas']}, "
...
def _market_day_shape(df: pd.DataFrame, target_date: date) -> dict[str, Any]:
    ...
    areas = sorted(str(area) for area in day_df["area"].dropna().unique())
    expected_area_df = day_df.loc[day_df["area"].isin(EXPECTED_AREAS)]
    periods_by_area = expected_area_df.groupby("area")[
    ...
    has_expected_areas = EXPECTED_AREAS.issubset(set(areas))
    expected_areas_complete = (
        has_expected_areas
        and len(periods_by_area) >= len(EXPECTED_AREAS)
        and all(
            int(periods_by_area.get(area, 0)) == expected_period_count
...
def main(
    *,
    ...
    """Poll and upsert the current PJM hourly demand bid publication."""
    ...
    try:
        ...
        run_logger.section("Waiting for complete demand bid data ...")
