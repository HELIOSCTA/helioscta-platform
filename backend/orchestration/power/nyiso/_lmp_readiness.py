"""Readiness helpers for NYISO LBMP workflows."""
...
def emit_lmp_data_availability_events(
    *,
    ...
    """Emit one readiness event per complete NYISO LBMP operating date."""
    if df.empty:
        logger.info("No NYISO LBMP rows available for readiness emission")
        return []
    ...
    if missing_columns:
        raise ValueError(
            "Cannot assess NYISO LBMP data readiness; missing columns: "
...
def emit_lmp_data_availability_event_for_date(
    *,
    ...
    if not is_complete:
        logger.warning(
            "Skipping NYISO LBMP readiness event for %s; incomplete rows "
    ...
    payload = {
        ...
        "market_clock": LOCAL_MARKET_TIMEZONE,
    }
