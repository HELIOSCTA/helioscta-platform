"""Readiness helpers for MISO LMP workflows."""
...
def emit_lmp_data_availability_events(
    *,
    ...
    """Emit one readiness event per complete MISO LMP operating date."""
    if df.empty:
        logger.info("No MISO LMP rows available for readiness emission")
        return []
    ...
    if missing_columns:
        raise ValueError(
            "Cannot assess MISO LMP data readiness; missing columns: "
...
def emit_lmp_data_availability_event_for_date(
    *,
    ...
    if not is_complete:
        logger.warning(
            "Skipping MISO LMP readiness event for %s; incomplete rows "
    ...
    window_start, window_end = _lmp.market_day_window_utc(business_date)
    payload = {
    ...
    return emit_data_availability_event(
        ...
        window_end=utc_timestamp(window_end),
        scope=scope,
        grain=grain,
        source_table=source_table,
        row_count=row_count,
...
def expected_period_count_for_date(
    ...
    *,
    ...
    _ = business_date
    return int(timedelta(days=1) / timedelta(minutes=interval_minutes))
