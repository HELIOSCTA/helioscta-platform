"""Shared orchestration helpers for MISO LMP workflows."""
...
from backend.orchestration.power.miso import _lmp_readiness
...
class DataNotYetAvailable(Exception):
    """Raised when MISO has not published a complete LMP operating date."""
...
def wait_for_complete_data_logged(
    *,
    ...
    endpoint = scrape_module.ENDPOINT_TEMPLATE.format(
        operating_date=_lmp.coerce_operating_date(operating_date).isoformat()
    )
    parsed_url = urlsplit(f"{data_exchange_client.BASE_URL.rstrip('/')}/{endpoint}")
    ...
    *,
    ...
    if not shape["is_complete"]:
        raise DataNotYetAvailable(
            "MISO LMPs are not complete for "
