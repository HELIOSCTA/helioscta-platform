"""Readiness helpers for SPP LMP workflows."""
...
def emit_lmp_data_availability_events(
    *,
    ...
    """Emit one readiness event per complete SPP LMP operating date."""
    if df.empty:
        logger.info("No SPP LMP rows available for readiness emission")
        return []
    ...
    if missing_columns:
        raise ValueError(
            "Cannot assess SPP LMP data readiness; missing columns: "
...
def emit_lmp_data_availability_event_for_date(
    *,
    ...
    if not is_complete:
        logger.warning(
            "Skipping SPP LMP readiness event for %s; incomplete rows "
