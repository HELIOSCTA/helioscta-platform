"""Shared orchestration helpers for SPP LMP workflows."""
...
from backend.orchestration.power.spp import _lmp_readiness
...
class DataNotYetAvailable(Exception):
    """Raised when SPP has not published a complete LMP operating date."""
...
def run_lmp_workflow(
    *,
    ...
    """Run one SPP LMP workflow and emit readiness events."""
    ...
    try:
        ...
        if events:
            ...
            run_logger.info(
                "No complete SPP LMP operating date detected; "
...
def wait_for_complete_data_logged(
    *,
    ...
    parsed_url = urlsplit(scrape_module.ENDPOINT_URL)
