"""Orchestrate SPP day-ahead hourly LMPs."""
...
from backend.orchestration.power.spp import _lmp_readiness, _lmp_workflow
from backend.scrapes.power.spp import _lmp
from backend.scrapes.power.spp import da_lmps as scrape
...
DATASET_NAME = "spp_da_lmps"
DATA_SCOPE = "hubs_spp_north_south"
...
def main(
    ...
    """Run the SPP DA LMP workflow and emit readiness events."""
...
def _notify_da_email_release_events(
    *,
    ...
    if run_mode != "scheduled":
        run_logger.info("Skipping SPP DA release emails outside scheduled mode.")
        return 0
    if not events:
        return 0
    ...
    try:
        for event in events:
            enqueued_rows = (
                email_notifications.enqueue_spp_da_lmp_release_notifications(
                    event=event,
                    database=database,
                )
            )
        ...
        if not email_notifications.notifications_enabled():
            run_logger.info(
                "SPP DA release email notifications "
        ...
        run_logger.exception(
            "SPP DA release email notification handling failed; "
