"""Orchestrate ISO-NE day-ahead hourly cleared demand."""
...
from backend.scrapes.power.isone import da_hrl_cleared_demand as scrape
...
DATASET_NAME = "isone_da_hrl_cleared_demand"
...
def main(
    ...
    """Run the ISO-NE day-ahead hourly cleared demand workflow."""
    ...
    try:
        ...
        if events:
            ...
            run_logger.info(
                "No complete ISO-NE DA cleared demand date detected; "
...
def _emit_data_availability_events(
    ...
    """Emit one readiness event per complete ISO-NE DA cleared demand date."""
    if df.empty:
        logger.info("No ISO-NE DA cleared demand rows available")
        return []

    missing_columns = {"date", "hour_ending"} - set(df.columns)
    if missing_columns:
        raise ValueError(
            "Cannot assess ISO-NE DA cleared demand readiness; missing columns: "
...
def _emit_data_availability_event_for_date(
    *,
    ...
    if not is_complete:
        logger.warning(
            "Skipping ISO-NE DA cleared demand readiness event for %s; "
