"""Orchestrate ISO-NE hourly actual scheduled interchange."""
...
from backend.scrapes.power.isone import rt_hrl_scheduled_interchange as scrape
...
def main(
    ...
    """Run the ISO-NE hourly interchange workflow."""
    ...
    try:
        ...
        if events:
            ...
            run_logger.info(
                "No complete ISO-NE hourly interchange date detected; "
...
def _emit_data_availability_events(
    ...
    """Emit one readiness event per complete ISO-NE interchange date."""
    if df.empty:
        logger.info("No ISO-NE interchange rows available")
        return []

    required_columns = {"local_date", "local_hour_ending", "interface_name"}
    missing_columns = required_columns - set(df.columns)
    if missing_columns:
        raise ValueError(
            "Cannot assess ISO-NE interchange readiness; missing columns: "
    ...
    current_df = current_df.dropna(
        subset=["local_date", "local_hour_ending", "interface_name"]
    )
...
def _emit_data_availability_event_for_date(
    *,
    ...
    entity_count = int(date_df["interface_name"].nunique())
    period_count = int(date_df["local_hour_ending"].nunique())
    periods_per_entity = date_df.groupby("interface_name")[
        "local_hour_ending"
    ].nunique()
    ...
    duplicate_entity_period_rows = int(
        date_df.duplicated(["interface_name", "local_hour_ending"]).sum()
    )
    ...
    if not is_complete:
        logger.warning(
            "Skipping ISO-NE interchange readiness event for %s; incomplete rows "
            "(rows=%s, interfaces=%s, periods=%s, expected_periods=%s, "
