...
from backend.backfills.power.caiso import rt_lmps


def test_caiso_rt_lmps_backfill_dry_run():
    result = rt_lmps.main(
    ...
    assert result.pipeline_name == "rt_lmps"
...
def test_caiso_rt_lmps_backfill_rejects_too_large_window():
...
def test_caiso_rt_lmps_backfill_calls_orchestration_with_backfill_metadata(
    monkeypatch,
):
    ...
    monkeypatch.setattr(rt_lmps.workflow, "main", fake_workflow_main)
