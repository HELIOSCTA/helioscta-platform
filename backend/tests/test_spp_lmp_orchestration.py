...
from backend.orchestration.power.spp import (
    _lmp_readiness,
    _lmp_workflow,
    da_lmps,
    rt_lmps_prelim,
...
def test_spp_lmp_event_key_and_expected_periods():
    assert (
        da_lmps._data_availability_event_key(date(2026, 8, 5))
        == "spp_da_lmps:data_ready:2026-08-05:hubs_spp_north_south"
    )
    assert da_lmps._expected_period_count_for_date(date(2026, 8, 5)) == 24
    assert da_lmps._expected_period_count_for_date(date(2026, 3, 8)) == 23
    assert rt_lmps_prelim._expected_period_count_for_date(date(2026, 8, 5)) == 288
...
def test_spp_lmp_scheduled_target_dates_use_central_market_date():
    now = pd.Timestamp("2026-08-05T21:00:00Z")
...
def test_spp_da_lmps_emits_readiness_for_complete_default_hubs(monkeypatch):
    ...
    assert events == [
        {
            "id": 1,
            "event_key": (
                "spp_da_lmps:data_ready:2026-08-05:"
                "hubs_spp_north_south"
            ),
            "created": True,
        }
    ]
    event = captured[0]
    assert event["dataset"] == "spp_da_lmps"
    assert event["source_system"] == "spp"
    assert event["availability_type"] == "data_ready"
    assert event["business_date"] == date(2026, 8, 5)
    assert event["scope"] == "hubs_spp_north_south"
    assert event["grain"] == "operating_date_hour_node"
    assert event["source_table"] == "spp.da_lmps"
    ...
    assert event["payload"]["market_clock"] == "America/Chicago"


def test_spp_da_main_passes_release_notification_handler(monkeypatch):
...
def test_spp_da_release_email_notifications_are_idempotent_and_sent(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "enqueue_spp_da_lmp_release_notifications",
        fake_enqueue,
    )
    monkeypatch.setattr(
        da_lmps.email_notifications,
        ...
        events=[
            {
                "id": 1,
                "event_key": (
                    "spp_da_lmps:data_ready:2026-08-06:"
                    "hubs_spp_north_south"
                ),
            }
        ],
    ...
    assert calls[0]["event"]["event_key"] == (
        "spp_da_lmps:data_ready:2026-08-06:hubs_spp_north_south"
    )
    assert calls[0]["database"] == "stage_db"


def test_spp_rt_lmps_skips_readiness_when_a_hub_is_missing(monkeypatch):
    captured: list[dict[str, object]] = []
...
def test_spp_da_lmps_fetch_complete_market_day_suppresses_per_attempt_logs(
    monkeypatch,
):
    captured: dict[str, object] = {}
...
def test_spp_da_lmps_wait_logs_one_resolved_poll_row(monkeypatch):
    ...
    assert log["operation_name"] == "spp_da_lmps_poll"
...
def test_spp_rt_fetch_complete_market_day_treats_404_as_not_available(monkeypatch):
    def fake_pull(**_kwargs):
        raise _lmp.SPPPortalDataNotAvailable("not found", status_code=404)
