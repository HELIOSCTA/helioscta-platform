...
-- backend.scrapes.nav.positions or backend.orchestration.nav.positions.
--
...
-- Safe rerun: backend upserts by the primary key, preserving distinct NAV
-- uploads for the same NAV date.
-- Note: this source table is raw-only. Product-code, product-group, contract,
-- instrument-type, and normalization-status fields are derived by read-only SQL
-- at query time and are not persisted here.
