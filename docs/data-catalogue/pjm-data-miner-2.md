...
| load_frcstd_hist | Historical Load Forecasts | Load Forecast | Retired; table dropped from `helios_prod` | pjm.load_frcstd_hist |
...
| Retired | load_frcstd_hist | Historical Load Forecasts | none | none | Removed from current code | Not retained in hot production storage | evaluated_at_utc, evaluated_at_ept, forecast_hour_beginning_utc, forecast_hour_beginning_ept, forecast_area |
...
- Runtime: none in the current tree; the previous scrape and reference SQL are
  recoverable from git history.
- Scheduling stance: retired from current code. Reintroduce only if an approved
