...
export const GET = observedJsonRoute(ROUTE_CONFIG, async () => {
  ...
  if (!rows.length) {
    return {
      status: 404,
      payload: { error: "No PJM Meteologica load forecast summary data is available" },
      headers: { "Cache-Control": "no-store", "X-Pjm-Meteologica-Forecast-Explorer-Cache": "MISS" },
    };
  }
  ...
  return {
    ...
    headers: { "Cache-Control": CACHE_HEADER, "X-Pjm-Meteologica-Forecast-Explorer-Cache": "MISS" },
    rowCount: rows.length,
    dataAsOf: asOf,
  };
});
