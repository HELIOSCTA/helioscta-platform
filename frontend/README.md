...
Those fields are computed in read-only SQL from raw
`nav.positions` columns; they are not stored in `nav.positions`. Fund and
account are coverage fields on the grouped row, not grouping keys. The capped
raw rows include the original NAV source columns plus query-time normalized
fields for inspection. The page also exposes local SQL downloads for rule
validation and grouped-position checks. It does not mutate data or create a
frontend cache table.
...
## Local Power Sparks Source Contract

The Power Sparks local view reads non-option ICE settlement marks with
...
## Local Power ICE Settles Source Contract

The Power ICE Settles local view reads PJM short-term and monthly settlement marks
