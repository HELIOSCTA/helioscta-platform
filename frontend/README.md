...
GET /api/dev/nav-positions?fund=all
...
The route `GET /api/dev/nav-positions` reads a promoted dbt mart from
`frontend/sql/nav-positions/marts/all_history.sql`. That file is generated by
dbt from
...
`date=YYYY-MM-DD`, `fund`, `accountGroup`, and `product`.
Without `date`, it selects the latest NAV date and latest upload per fund. The
response returns a product summary aggregated by normalized product identity:
`product_code`, `product_group`, `product_region`, `underlying_product_code`,
`contract_yyyymm`, `contract_day`, `put_call`, and `normalized_strike_price`.
Those fields are computed by dbt read-only SQL from raw `nav.positions`
columns; they are not stored in `nav.positions`. Fund and account are coverage
fields on the grouped row, not grouping keys. It does not mutate data or create
a frontend cache table. The page pivots this response client-side into a
Short Term Power-style ladder with products as rows and net quantity by
contract bucket as columns: short-term day/week buckets first, followed by
monthly futures columns labelled `YYYY-MM`. The NAV Snapshot selector chooses
the source position file date; the Anchor Date chooses the date used for
`Bal Day`, `Next Day`, and week buckets. The page exposes one Account filter
backed by the NAV `fund_code` values.

Raw source rows are available only through the local debug modal, which calls
`GET /api/dev/nav-positions?mode=debug&limit=500` with the same bounded filters.
Clicking a populated ladder cell opens the same modal with additional
product-identity and contract-bucket drilldown filters.
