...
GET /api/pjm-product-settles-summary?component=total&rtSource=verified
...
The Power Daily Settles page accepts `iso=pjm|ercot|isone|caiso|miso|spp|nyiso` and exposes
...
completeness status details. Adders and reserves remain owned by the Power Daily Settles
...
## Power Product Settles Source Contract

The Power Product Settles page (`/?section=pjm-historical-settlements`) reads
historical hourly PJM LMPs with `helios_readonly` from `pjm.da_hrl_lmps`,
`pjm.rt_hrl_lmps`, and `pjm.rt_unverified_hrl_lmps`. The default tab is
MTD Summary; `/?section=pjm-term-bible` opens the same page on the Term Bible
tab.
...
The route `GET /api/pjm-product-settles-summary` accepts bounded params:
`end=YYYY-MM-DD`, `component=total|energy|congestion|loss`, and
`rtSource=verified|unverified`. Without `end`, it defaults to PJM market
month-to-date: the first day of the month through yesterday in
America/New_York. When `end` is supplied, the start date remains the first day
of the end date's month. Future end dates are capped to yesterday EPT.

The summary uses `frontend/lib/powerPricing/ice_power_registry.json` only as
product metadata. It includes active PJM entries with `pjm_pnode_name` from
`futures.pjm`, `shortTerm.pjm`, and `productDictionary`, deduping mirrored
registry rows across those sources. Settlement values are derived from PJM LMP
tables, not ICE settlement prices. Registry Peak/ONPEAK products map to
`5x16`, off-peak products with weekend/holiday hours map to `wrap`, and
7x8-style off-peak products map to `7x8`. The response returns Product,
Contract, Hub, Market, Shape, Hours, MTD average, priced-day observations,
expected day/hour coverage, status, ICE link metadata, source tables, and
route freshness fields.

The compatibility route `GET /api/pjm-historical-settlements` accepts bounded params:
...
The Term Bible tab reuses `GET /api/pjm-term-bible`. Clicking an MTD Summary
row pins that product's PJM pnode, DA/RT market, selected component, detail
month, and mapped period into Term Bible filters.
...
the page and production URL parsing falls back to the Power Daily Settles section.
